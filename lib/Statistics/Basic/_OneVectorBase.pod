...
something to calculate (or until the vector is full if
L<$ENV{NOFILL}|Statistics::Basic/$ENV{NOFILL}> is in effect).
...
either takes one argument -- which can either be an arrayref or a
L<Statistics::Basic::Vector> object -- or it can take a list of elements to
populate the vector with.
...
L<$ENV{IPRES}|Statistics::Basic/$ENV{IPRES}> for further information.

...
pretty value for the correlation (or C<n/a>, see
L<query()|Statistics::Basic::_TwoVectorBase/query()> above).

If evaluated as a number, it will try to return the raw result of
L<query()|Statistics::Basic::_TwoVectorBase/query()>, possibly turning the
resulting C<undef> (if applicable) into a 0 in the process -- note that Perl

...
Here, the covariance would need to be recalculated by the correlation.  But if
we instead build vectors first then that wouldn't happen.
...
In the second case, trying to use C<$multimodal> as a number will C<croak> an
error.  But it interpolates just fine.
...
This allows you to set the vector to a known state.  It takes either arrayrefs
or vector objects.
